 ...
 *
 * En el ejercicio anterior usamos un semáforo binario, sin embargo ahora vamos a
 * usar un semáforo contador.
 */
 ...
 * En este caso el semáforo es contador.
 */
...
int main (){
	 ...
	 * Nuestro semáforo va a tener un conteo máximo de 10 y además
	 * va a estar inicializado en 0.
	 */
...
static uint32_t ulExampleInterruptHandler( void ){
	...
	/* Vamos a hacer GIVE del semaforo muchas veces, yua que el primer GIVE va a desbloquear
	 * la tarea. Y los dos siguientes son para demostrar que se puede apilar varios semaforos de
	 * tal manera que se procesen sin que se pierda ningun evento. Esto simula multiples interrupciones
	 * siendo procesadas por el procesador.
	 */
